...
public class Kitchen {
    ...
    public void makeTaco(int number) throws NoCookException, NoIngredientException {
        if (!cookReady) throw new NoCookException("Cook isn't ready.");
        if (ingredient - (INGREDIENT_PER_TACO * number) < 0) throw new NoIngredientException("Not enough ingredients.");
    ...
    public void buyIngredients(int amount) throws NotEnoughMoneyException {
        if (balance - (DOLLAR_PER_INGREDIENT * amount) < 0) throw new NotEnoughMoneyException();
